...
- Keep Inspiredesign media-analysis dependency/status guidance in `opendevbrowser-best-practices` aligned with `docs/CLI.md`, `docs/SURFACE_REFERENCE.md`, `docs/DEPENDENCIES.md`, `docs/TROUBLESHOOTING.md`, public-surface source, generated manifests, and media-analysis dependency guidance tests. Preserve the contract: optional host FFmpeg/FFprobe tools, no bundled static binaries or default downloads, env then config then `PATH` resolution plus common absolute directory fallback for implicit `PATH`-source ENOENT misses only, invalid env/config diagnostics that do not fall back, `status-capabilities.host.mediaAnalysis` as diagnostic/preflight only, missing binaries degrading `media-analysis.json` only, and `pin-media-index.json` plus `motion-evidence.json` as authority surfaces.
