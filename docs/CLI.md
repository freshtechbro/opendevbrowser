...
cd "$WORKDIR"
npm init -y
npm install <public-repo-root>/opendevbrowser-0.0.38.tgz
...
- `--output-dir` is a workflow root, not the final bundle directory. Omit it for routine runs and inspect the returned `artifact_path` first; when omitted, persisted artifacts land under `.opendevbrowser/inspiredesign/<runId>`. If a wrapper must pass an explicit root, prefer `--output-dir .opendevbrowser`.
...
Release gate source of truth: `docs/RELEASE_RUNBOOK.md` and `docs/RELEASE_0.0.38_EVIDENCE.md`.
