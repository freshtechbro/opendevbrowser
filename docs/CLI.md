...
Generated help is the primary first-contact inventory and onboarding surface. Agent runs should start with `opendevbrowser_prompting_guide` or `opendevbrowser_skill_load opendevbrowser-best-practices "quick start"` before low-level browser commands, then load `opendevbrowser_skill_load opendevbrowser-best-practices "validated capability lanes"` when they need the current transcript, evidence-gated research, shopping, browser replay, desktop observation, and browser-scoped computer-use workflows named in `src/cli/onboarding-metadata.json`. Load `opendevbrowser_skill_load opendevbrowser-design-agent "canvas-contract"` immediately after that baseline for frontend, screenshot-to-code, or `/canvas` design work. Load `opendevbrowser_skill_load opendevbrowser-motion-design "quick start"` after design-agent for motion-heavy UI work, animation systems, scroll motion, gesture motion, reduced-motion audits, or motion release evidence. Use continuity guidance only for long-running handoff or compaction.
...
- `/canvas` (design-canvas channel): typed design-canvas protocol; see `docs/SURFACE_REFERENCE.md` for all `41` command names and envelope contracts.
...
npx opendevbrowser product-video run --product-name "Sample Product" --provider-hint shopping/amazon --browser-mode extension --use-cookies --challenge-automation-mode browser_with_helper
```
...
- `--output-dir` is a workflow root, not the final bundle directory. Omit it for routine runs so artifacts land under `.opendevbrowser/product-video/<runId>`; if a wrapper must pass an explicit root, prefer `--output-dir .opendevbrowser`.
...
- `--capture-mode` (`off|deep`; `run` forces deep capture for any explicit `--url`; `harvest` forces deep capture for non-Pinterest explicit `--url`; Pinterest-only harvest discovery and compatible Pinterest URL recovery force `captureMode=off` even when `--capture-mode deep` is requested)
...
- `--output-dir` is a workflow root, not the final bundle directory. Omit it for routine runs so artifacts land under `.opendevbrowser/inspiredesign/<runId>`; if a wrapper must pass an explicit root, prefer `--output-dir .opendevbrowser`.
- Proven Pinterest image, GIF, and video pins require manifest-backed pin-media evidence for product-ready canonical pin-media harvests. Screenshot evidence and screencast evidence remain useful capture or motion lanes, but they are not substitutes for `evidenceAuthority=pin_media_ready`. Video posters remain still-image fallback cues, and DOM/clone/deep capture is disabled for Pinterest harvest. Remote DOM media URLs are not product-readiness signals unless persisted first-party bytes appear in `pin-media-index.json`.
- `inspiredesign run` forces `captureMode=deep` for any explicit `--url`, including Pinterest URLs, so the workflow can collect DOM/layout diagnostics. `inspiredesign harvest` forces `captureMode=deep` for non-Pinterest explicit `--url` references, even when `--capture-mode off` is requested. Pinterest-only harvest discovery and compatible Pinterest URL recovery force `captureMode=off` even when `--capture-mode deep` is requested.
...
- `media-analysis.json` is a deterministic design-fact surface emitted from trusted saved pin media after finalization. It can describe dimensions, tone, quantized palette, layout posture, OCR-free typography structure, text-region geometry, and sampled GIF/video motion facts including `motionSignature`, but it never grants product readiness or replaces `pin-media-index.json` authority. media-analysis.json never satisfies product readiness. `motionSignature` is saved-media analysis only, not browser replay evidence or interaction choreography.
...
- Missing or invalid FFmpeg or FFprobe binaries degrade `media-analysis.json` only. They can reduce metadata, palette, tone, scene-score, or sampled motion detail, but they do not fail pin-media readiness and do not let `media-analysis.json` replace `pin-media-index.json` or `motion-evidence.json` authority. Empty `motion-evidence.json` means no authoritative browser replay screencast was captured; saved GIF/video motion can still appear in `media-analysis.json` as sampled saved-media facts.
...
- `canvas-plan.request.json` does not contain raw `mediaAnalysis`. Canvas receives only concise media-derived summaries through existing generation-plan and design-vector fields when readiness already permits Canvas continuation.
...
- `--browser-mode` applies to provider-backed reference retrieval. Screenshot, screencast, and deep diagnostic capture still use browser manager capture lanes.
...
- `inspiredesign run` returns a reusable design contract plus a typed handoff bundle. Canvas is primary only when product readiness is true, not merely when guidance readiness is `ready`.
- `inspiredesign harvest` extends the same workflow with query discovery, ranked references, screenshot, screencast, and pin-media evidence metadata, persisted evidence artifacts, typed recovery guidance, and Canvas follow-through only when product readiness is true.
- Prefer omitting `--output-dir` for routine research, shopping, inspiredesign, and product-video workflow runs. Omitted path-bearing workflow outputs write to `<invocation-or-workspace-root>/.opendevbrowser/<namespace>/<runId>` and include TTL metadata plus `bundle-manifest.json`. If a wrapper must pass an explicit workflow root, prefer `--output-dir .opendevbrowser`; the runtime appends the namespace and run id, so Inspiredesign lands at `.opendevbrowser/inspiredesign/<runId>`. Use external roots only intentionally, such as temp cleanup roots, release proof outputs, debug or audit evidence, screenshots, and screencast replay directories. The CLI rejects blank `--output-dir` values and resolves relative paths from the invocation directory before daemon dispatch. CLI runs use the process invocation directory. Direct daemon RPC uses `core.workspaceRoot/.opendevbrowser`, and direct OpenCode tools use the injected `deps.workspaceRoot/.opendevbrowser`; `src/index.ts` wires that value from `core.workspaceRoot`. Direct provider workflow calls without an injected workspace root keep the provider-level `process.cwd()/.opendevbrowser` fallback. Namespaces are `research`, `shopping`, `inspiredesign`, and `product-video`. Direct tool or daemon callers can still pass an explicit output directory for caller-specific placement.
...
- JSON output includes `host.mediaAnalysis` for FFmpeg and FFprobe availability, source, version, and limitation details before Inspiredesign media-analysis runs.
...
`canvas.session.open` -> inspect the handshake (`planStatus`, `preflightState`, `generationPlanRequirements.allowedValues`, `generationPlanIssues`, `guidance.recommendedNextCommands`, and `guidance.nextStepGuidance` when present) -> `canvas.plan.set` -> if accepted, follow returned guidance into `canvas.document.patch` (including `governance.update` blocks) -> `canvas.preview.render` -> `canvas.feedback.poll` -> `canvas.document.save` or `canvas.document.export`.
If `canvas.plan.set` fails with `generation_plan_invalid`, inspect the returned `details.missingFields`, `details.issues`, `guidance.paramsExamples`, `guidance.fieldExamples`, `guidance.validationChecks`, and `guidance.doNotProceedIf`, then repair and resubmit `canvas.plan.set`. `canvas.plan.get` and `canvas.capabilities.get` are diagnostic on that failure path, not required success-path checkpoints.
Additional same-session clients use `canvas.session.attach` with `attachMode=observer` or `attachMode=lease_reclaim`.
Workspace orchestration is additive above child sessions: create or attach normal child sessions first, then use `canvas.workspace.open`, `canvas.workspace.status`, `canvas.workspace.child.add`, `canvas.workspace.child.execute`, `canvas.workspace.child.close`, and `canvas.workspace.close` to coordinate child refs without duplicating document contents. Child commands still execute through the existing child `CanvasManager` session, lease, document store, preview, feedback, and code-sync semantics.
Unless `params.repoRoot` is provided explicitly, the CLI injects the caller cwd as the canvas session repo root. Relative `canvas.document.save`, `canvas.document.export`, `canvas.code.*`, and workspace manifests resolve against that session root even when the daemon is launchd-owned or started from another working directory.
...
# Repair an invalid or incomplete generation plan by following returned paramsExamples
...
uses direct CDP when possible and falls back to relay in extension sessions. New captures and stored payloads use Annotation V2
compact handoff by default: `schemaVersion: 2`, `compact.screenshotMode="none"`, redaction metadata, selector bundles,
canvas identity when available, and screenshot-free shared inbox storage. `annotate --stored` resolves the shared repo-local
...
Release gate source of truth: `docs/RELEASE_RUNBOOK.md` and `docs/RELEASE_0.0.37_EVIDENCE.md`.
...
`discoveryPort` is the HTTP `/config` discovery listener used by the extension to learn the active relay port. Keep it at `8787` for normal extension discovery, or set it to the same isolated value as `relayPort` when running a separate daemon on isolated config/cache roots.
