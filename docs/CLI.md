...
- `--capture-mode` (`off|deep`; `run` forces deep capture for any explicit `--url`; `harvest` forces deep capture for non-Pinterest explicit `--url`; Pinterest-only harvest discovery and compatible Pinterest URL recovery keep deep diagnostics opt-in)
...
- Proven Pinterest image pins use screenshot evidence or manifest-backed pin-media evidence, proven Pinterest video pins use screencast motion evidence for motion and video-poster pin-media evidence for still-image cues, and DOM/clone/deep capture remains diagnostic capture only for Pinterest harvest discovery. Remote DOM media URLs are not product-readiness signals unless persisted first-party bytes appear in `pin-media-index.json`.
- `inspiredesign run` forces `captureMode=deep` for any explicit `--url`, including Pinterest URLs, so the workflow can collect DOM/layout diagnostics. `inspiredesign harvest` forces `captureMode=deep` for non-Pinterest explicit `--url` references, even when `--capture-mode off` is requested. Pinterest-only harvest discovery and compatible Pinterest URL recovery use deep capture only when `--capture-mode deep` is explicitly requested.
...
- The ready follow-through path is explicit: read `advanced-brief.md`, `meta-prompt.md`, `ranked-references.json`, and screenshot, screencast, or pin-media evidence first; load `opendevbrowser_skill_load opendevbrowser-best-practices "quick start"`, `opendevbrowser_skill_load opendevbrowser-design-agent "canvas-contract"`, and `opendevbrowser_skill_load opendevbrowser-motion-design "quick start"`; continue only if `canvas-plan.request.json` exists; open a Canvas session; fill the session ids in `canvas-plan.request.json`; run `opendevbrowser canvas --command canvas.plan.set --params-file ./canvas-plan.request.json`; confirm `planStatus=accepted`; then patch only the governance blocks called out by `design-agent-handoff.json`.
