...
cd "$WORKDIR"
npm init -y
npm install <public-repo-root>/opendevbrowser-0.0.36.tgz
...
- Proven Pinterest image pins use screenshot evidence or manifest-backed pin-media evidence, and proven Pinterest video pins use manifest-backed video pin-media bytes when actual video is available. Video posters remain still-image fallback cues, screencast evidence remains the motion-ready lane, and DOM/clone/deep capture is disabled for Pinterest harvest. Remote DOM media URLs are not product-readiness signals unless persisted first-party bytes appear in `pin-media-index.json`.
...
- Harvest outputs separate operational completion from product readiness. `success:true` or `ok:true` means the command completed and wrote artifacts; `productSuccess`, `harvestReadiness`, `readiness`, `rankedReferenceCount`, `evidenceAuthority`, and `artifactAuthority` determine whether the harvest is usable as design evidence.
- Treat readiness as the gate between artifact completion and design readiness. Continue to Canvas only when `nextStepGuidance.readiness` is `ready`, ranked references are non-empty, Pinterest references have manifest-backed `snapshot_ready`, `motion_ready`, or `pin_media_ready` evidence, and no `doNotProceedIf` condition applies.
- Pinterest product readiness is artifact-first: canonical pin URLs rank only when their screenshot, screencast, or first-party pin-media artifact is captured, persisted, present in its manifest-backed index, and free of blocking warnings. The exact `login_or_challenge_state` and strict byte-backed `interface_chrome_shell` diagnostics are non-blocking only for trusted first-party manifest-backed pin-media bytes; broader login, challenge, captcha, search-shell, promoted, ad, blank, tiny, or chrome-only blockers still demote readiness. Multimodal agents reason over those artifacts directly.
...
Release gate source of truth: `docs/RELEASE_RUNBOOK.md` and `docs/RELEASE_0.0.36_EVIDENCE.md`.
