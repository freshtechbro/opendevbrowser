...
- `opendevbrowser_inspiredesign_run` - Run the inspiredesign workflow directly, including provider-scoped URL recovery, harvest query discovery, screenshot evidence for image pins, and screencast evidence for video pins.
...
- Inspiredesign harvest primary capture is evidence-first for Pinterest: proven image pins use screenshot evidence, proven video pins use screencast motion evidence, and DOM/clone/deep capture remains diagnostic capture only rather than a product-readiness signal.
...
- Inspiredesign harvest artifacts: `visual-evidence.json`, `screenshot-index.json`, `motion-evidence.json`, `ranked-references.json`, and `meta-prompt.md` are emitted with screenshot PNGs under `visual-evidence/<referenceId>/viewport.png` and motion artifacts under `motion-evidence/<referenceId>/` when video evidence is captured. JSON remains metadata-only with artifact-relative paths, hashes, byte counts, viewport metadata when available, reference id and URL, and warnings.
...
- Continue to Canvas only when `nextStepGuidance.readiness` is `ready`, ranked references are non-empty, and Pinterest references have `snapshot_ready` or `motion_ready` evidence. For `needs_recovery`, `blocked`, or `diagnostic_only`, follow recovery-first guidance and do not treat emitted artifacts as design-ready.
- Pinterest product readiness is snapshot-first: canonical pin URLs rank only when their screenshot or screencast artifact is captured, persisted, and free of login, challenge, search-shell, blank, tiny, or chrome-only blockers.
