...
- `package.json`: no dependency or script changes required for the immediate override scope
- `tsconfig.json`: no compiler-setting changes required
- `eslint.config.js`: no lint-rule changes required
- `vitest.config.ts`: no test-runner config changes required
- Vite config: no Vite config exists in the public repo, so no Vite update is required
...
| 1.1 | 2026-03-23 | Added config-surface audit notes and synchronized the plan with the public docs, dependency inventory, and cutover checklist scope |
