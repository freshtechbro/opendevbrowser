...
`CanvasManager` lives in `src/browser/canvas-manager.ts` and composes dedicated session-sync, code-sync, starter-catalog, workspace orchestration, and runtime-preview bridge helpers while delegating document, export, framework-adapter, library-adapter, plugin, starter, kit, and token primitives to `src/canvas/` plus deterministic Figma import helpers under `src/integrations/figma/`.
Daemon-backed `canvas` CLI requests inject the caller worktree as `repoRoot`; `CanvasManager` persists that root per canvas session so relative document saves, exports, imported asset materialization, code-sync manifest/source paths, and workspace manifests resolve against the caller repo instead of the daemon process cwd.
`AgentInbox` provides repo-local, chat-scoped delivery for explicit popup/canvas/in-page annotation sends. Core bootstrap registers the relay `store_agent_payload` handler, enqueues screenshot-free Annotation V2 compact payloads, and supports shared `annotate --stored` retrieval.
...
- Typed guidance recipes distinguish artifact completion from design readiness. Workflow payloads can emit `nextStepGuidance` with `readiness`, `reasonCode`, `primaryAction`, command examples, params examples, validation checks, fallback policy, and `doNotProceedIf` blockers.
- Browser-native site recipes sit beside provider adapters for visually driven or logged-in site workflows. Pinterest is the first recipe for `social/pinterest`; it is not registered as a default full social provider and does not widen to unrelated providers without user confirmation.
...
- Design canvas surfaces expose `canvas.execute` / `opendevbrowser_canvas` / `opendevbrowser canvas` and are layered as:
  - `session handshake + attach` (`canvas.session.open`, `canvas.session.attach`, `canvas.capabilities.get` after a session exists) for governance, plan requirements, same-user observer joins, explicit lease reclaim, `generationPlanRequirements.allowedValues`, `generationPlanIssues`, runtime-returned `guidance.recommendedNextCommands`, and typed repair envelopes with `nextStepGuidance`, params examples, field examples, validation checks, and do-not-proceed blockers
  - `workspace orchestration` (`canvas.workspace.open`, `canvas.workspace.status`, `canvas.workspace.child.add`, `canvas.workspace.child.execute`, `canvas.workspace.child.close`, `canvas.workspace.close`) for refs-only coordination across child sessions, child-route execution through existing leases, duplicate lease/document/repo-path/code-sync binding guards, preview budget state, latency telemetry, memory samples, and coordinator close that preserves child sessions
  ...
  - `live editor + preview + overlay` (`canvas.tab.open`, `canvas.overlay.mount`, `canvas.preview.render`, `canvas.preview.refresh`) for browser-backed iteration; extension mode uses `extension/canvas.html` as the same-origin infinite-canvas host with pages, layers, properties, token collection or mode authoring, alias or binding controls, keyboard shortcuts, extension-stage region annotation, and workspace/child scoped 4x2 shell state, while preview targets prefer `bound_app_runtime` reconciliation for opted-in bindings and fall back to core-generated `canvas_html` projections when runtime bridge preflight fails or no bound sync root exists
...
- **Release checklist** in `docs/RELEASE_RUNBOOK.md` with evidence tracking in the current version-scoped release ledger (for this cycle: `docs/RELEASE_0.0.38_EVIDENCE.md`).
