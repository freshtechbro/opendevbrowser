...
- Defaults: Mode A, localhost only, redaction enabled, maxChars, per-project cache.
- Reads `opendevbrowser` block from OpenCode config.
- Validates and merges user overrides.
